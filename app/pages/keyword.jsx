...
const BackIcon = ({ size = 18, color = '#fff' }) => (
...
const KeywordCard = ({ keyword, isSmallDevice }) => {
  const [isChecked, setIsChecked] = useState(false);

  const getStatusStyle = (status) => {
    switch (status) {
      case 'live':
        return { bg: '#e6f9f4', color: '#00a878' };
      case 'updating':
        return { bg: '#fff7e0', color: '#d48a00' };
      case 'done':
        return { bg: '#f0f0f0', color: '#777' };
      default:
        return { bg: '#f0f0f0', color: '#777' };
    }
  };
  ...
  return (
    <View className="flex-1 bg-surface2">
      ...
      >
        ...
        <TouchableOpacity>
          <Text className="text-primary font-medium" style={{ fontSize: 12.5 }}>
            Select all
          </Text>
        </TouchableOpacity>
      </View>
