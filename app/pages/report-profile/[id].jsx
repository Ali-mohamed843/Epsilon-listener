...
import { FilterChips, KpiCard, FindingsCard, SentimentWordCloud, DonutLegend, SectionTitle, SimpleBarChart } from '../../../components/ReportComponents';
...
export default function ProfileReportScreen() {
  ...
  const uiData = useMemo(() => {
    ...
    const commentPos = commentKeywords.percentage?.positive || 0; const commentNeg = commentKeywords.percentage?.negative || 0;
    const commentNeu = Math.max(0, 100 - commentPos - commentNeg); const postPos = postKeywords.percentage?.positive || 0; const postNeg = postKeywords.percentage?.negative || 0;
    ...
    const platformType = profile.type || 'facebook-profile'; const platformName = platformType.replace('-profile', '').charAt(0).toUpperCase() + platformType.replace('-profile', '').slice(1);
    const safeNum = (v) => Number(v) || 0; const transformEng = (f) => engagementGraph.map(d => ({ label: new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), value: safeNum(d[f]) }));
    ...
    return {
      ...
      commentPos, commentNeg, commentNeu, postPos, postNeg, posWords, negWords, hashtagList, platformName,
  ...
  const aiContextData = useMemo(() => uiData ? { name: uiData.profileName, platform: uiData.platformName, profileUrl: uiData.pageUrl, dateRange: uiData.dateRange, kpis: uiData.kpis.map(k => ({ label: k.label, value: k.value })), fakePercent: 0, realPercent: 100, totalAccounts: 0, riskLevel: uiData.riskLevel, commentsSentiment: { positive: uiData.commentPos.toString(), negative: uiData.commentNeg.toString(), neutral: uiData.commentNeu.toString() }, postsSentiment: { positive: uiData.postPos.toString(), negative: uiData.postNeg.toString() }, positiveWords: uiData.posWords, negativeWords: uiData.negWords, hashtags: uiData.hashtagList, platformMentions: [{ name: uiData.platformName, value: uiData.kpis[7]?.value || '0' }] } : null, [uiData]);
  ...
  const BadgeIcon = platformBadges[reportData.profile.type] || FacebookBadge; const avatarBg = platformGradients[reportData.profile.type] || '#1877f2';
  ...
  return (
    <View className="flex-1 bg-surface2">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 80 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="flex-row" style={{ gap: 10, marginBottom: 10 }}><SentimentWordCloud title="Positive" percentage={String(uiData.commentPos)} words={uiData.posWords} type="positive" /><SentimentWordCloud title="Negative" percentage={String(uiData.commentNeg)} words={uiData.negWords} type="negative" /></View>
