...
const HashtagWordCloudWebView = ({ hashtags = [], height = 400, onHashtagPress }) => {
  ...
  // Process hashtags - calculate size based on platformContents count
  ...
  const handleHashtagClick = (tag) => {
    ...
    } else if (tag.platformContents && tag.platformContents.length > 0) {
      if (tag.platformContents.length === 1) {
        Linking.openURL(tag.platformContents[0].permalink).catch(err => {
          console.error('Failed to open URL:', err);
        });
      } else {
        Alert.alert(
          tag.text,
          `${tag.platformContents.length} posts available`,
          [
            ...tag.platformContents.slice(0, 5).map((content, index) => ({
              text: `Open Post ${index + 1}`,
              onPress: () => Linking.openURL(content.permalink).catch(console.error),
            })),
    ...
    try {
      ...
      if (data.hashtagClicked) {
        const clickedTag = processedHashtags.find(
          (h) => h.text === data.hashtagClicked || h.id === data.hashtagClicked
        );
  ...
  const buildHtml = (wordList, zoom) => {
    if (wordList.length === 0) {
      return `
        ...
        <body>
          <div style="color: #9e859e; font-size: 14px;">No hashtags found</div>
        </body>
        </html>
      `;
    }
    ...
    const wordsWithSizes = wordList.map((w) => {
      ...
      return {
        ...
        color: `rgb(${r}, ${g}, ${b})`,
        id: w.id,
        hasLinks: w.platformContents && w.platformContents.length > 0,
        linkCount: w.platformContents?.length || 0,
      };
    });
    ...
    const wordsHtml = shuffledWords.map((w, i) => `
      <span 
        class="hashtag" 
        data-id="${w.id}"
        data-text="${w.text.replace(/"/g, '&quot;')}"
        data-links="${w.hasLinks}"
        data-count="${w.linkCount}"
        style="
          font-size: ${w.fontSize}px;
          font-weight: ${w.fontWeight};
          color: ${w.color};
          padding: ${Math.max(4, w.fontSize * 0.2)}px ${Math.max(6, w.fontSize * 0.3)}px;
...
<script>
  ...
  document.querySelectorAll('.hashtag').forEach(el => {
    el.addEventListener('click', function(e) {
      ...
      const hasLinks = this.getAttribute('data-links') === 'true';
      const linkCount = parseInt(this.getAttribute('data-count') || '0');
  ...
  document.addEventListener('touchmove', function(e) {
    if (e.touches.length === 2) {
      ...
      scale = Math.min(Math.max(pinchStartScale * ratio, 0.5), 3);
      applyZoom();
    }
  }, { passive: false });
  ...
  const cloudHtml = useMemo(
    () => buildHtml(processedHashtags, zoomLevel),
    [processedHashtags, zoomLevel]
  );

  // Dynamic height based on hashtag count
  ...
  return (
    ...
    >
      ...
      <View style={{ height: cloudHeight }}>
        <WebView
          key={`hashtag-cloud-${zoomLevel}`}
          source={{ html: cloudHtml }}
      ...
      {selectedHashtag && selectedHashtag.platformContents?.length > 0 && (
        <View
          style={{
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ fontSize: 11, color: '#059669', marginTop: 2 }}>
              {selectedHashtag.platformContents.length} posts available
            </Text>
          </View>
          <TouchableOpacity
